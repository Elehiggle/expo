...
- On `Picker.Item`, the `style` prop only applies `color`, `backgroundColor`, `fontFamily`, and `fontSize`. `backgroundColor` paints behind the label text, not the entire row. The top-level `color` and `fontFamily` props are still supported as aliases for the corresponding `style` values.
...
`fontFamily` accepts iOS font names (for example, `'Menlo'`) on iOS, and Compose generic families (`'monospace'`, `'serif'`, `'sansSerif'`, `'cursive'`) or fonts loaded with [`expo-font`](../../font) on Android. Use [`Platform.select`](https://reactnative.dev/docs/platform#select) when the value differs.
  ...
  return (
    <Picker selectedValue={language} onValueChange={value => setLanguage(value)}>
      ...
      <Picker.Item
        label="Swift"
        value="swift"
        style={{ color: '#d97706', fontFamily: serif, fontSize: 20 }}
        enabled={false}
      />
    </Picker>
  );
